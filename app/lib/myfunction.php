...
//Phân cấp danh mục sản phẩm - Đệ quy
...
function cateMenu($data, $char = '|--', $select = 0)
{
    foreach ($data as $cate) {
        if ($cate->id != 0 && $cate->id == $select) {
            echo "<option value=$cate->id selected='selected'>$char $cate->name</option>";
        } else {
            echo "<option value=$cate->id>$char $cate->name</option>";
        }
    }
}
...
//doi mau in dam keyword tim kiem
