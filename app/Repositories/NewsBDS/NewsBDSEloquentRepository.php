...
namespace App\Repositories\NewsBDS;
...
class NewsBDSEloquentRepository extends EloquentRepository implements NewsBDSRepositoryInterface
{
    ...
    {
        $getBDS = DB::table('categories as cate')
            ->rightjoin('news as n','n.Cate_id','=','cate.id')
            ->where('cate.cateType','BDS')
            ->get();
        return $getBDS;
    }
    ...
    public function getCreateAndEdit($inputFile, $id = 0)
    {
        if ($id == 0) {
            ...
            $news->save();
            $idNews = $news->id;
            return redirect()->route('newsBDS.index')->with(['thongbao' => 'Tin được tạo thành công']);
        } else {
            $news = $this->find($id);
            $news->view = 1;
            $news->title = $inputFile['txtName'];
    ...
    public function getDelete($id)
    {
        ...
        return redirect()->route('newsBDS.index')->with(['thongbao' => 'Tin tức đã được xóa, bất ngờ chưa!!!']);
    }
}