...
/*        $getPostandUn = array();
    ...
    {
        $getData = DB::table('fengshuis')->select('id', 'title', 'hot', 'feature', 'active', 'sort', 'relation')->get();
        return $getData;
    }
    ...
    public function getCreateAndEdit($inputFile, $id = 0)
    {
        if ($id == 0) {
            ...
            $news->save();
            return redirect()->route('fengshui.index')->with(['thongbao' => 'Tin được tạo thành công']);
        } else {
            $news = $this->find($id);
            $news->view = 1;
            if (isset($inputFile['slPost'])) {
