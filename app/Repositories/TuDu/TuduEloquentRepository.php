...
namespace App\Repositories\TuDu;
...
class TuduEloquentRepository extends EloquentRepository implements TuduRepositoryInterface
{
    ...
    public function getCreateAndEdit($inputFile, $id = 0)
    {
        if ($id == 0) {
            ...
            $news->save();
            return redirect()->route('tuyendung.index')->with(['thongbao' => 'Tin được tạo thành công']);
        } else {
            $news = $this->find($id);
            $news->title = $inputFile['txtName'];
            $news->alias= changeTitle($inputFile['txtName']);
    ...
    public function getDelete($id)
    {
        ...
        return redirect()->route('tuyendung.index')->with(['thongbao' => 'Tin đã được xóa, bất ngờ chưa!!!']);
    }
}