            ...
            <div class="card-box table-responsive">
                <table id="datatable-fixed-header" class="table table-striped table-bordered">
                    ...
                    @foreach($cates as $cate)
                        <tr>
                            <td>{{$cate->name}}</td>
                            <td><img src="{{asset('upload/thumbnail/'.$cate->metaName)}}" width="150"/></td>
                            <td><img src="{{asset('upload/thumbnail/'.$cate->description)}}" width="300"/></td>
                            <td style="text-align: center">{{$cate->weight}}</td>
                            <td>
                                <a href="{{route('catefurniture.edit',$cate->cateParen_id)}}"><button type="button" class="btn btn-icon waves-effect waves-light btn-warning">  <i class="fa fa-wrench"></i> </button></a>
                                {{--<a href="{{route('catefur.delete',$cate->cateParen_id)}}"><button type="button" class="btn btn-icon waves-effect waves-light btn-danger"> <i class="fa fa-remove"></i> </button></a>--}}
                            </td>
                        </tr>
                        @endforeach
                        </tbody>
